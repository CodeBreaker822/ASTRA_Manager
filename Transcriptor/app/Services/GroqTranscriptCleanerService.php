...
use App\Exceptions\GroqTranscriptCleanerException;
...
class GroqTranscriptCleanerService
{
    ...
    public function clean(string $text, array $timestamps = [], array $options = []): array
    {
        ...
        if (! is_string($content) || trim($content) === '') {
            throw new GroqTranscriptCleanerException(ServiceUserMessage::emptyCleanerResponse('Groq'));
        }
    ...
    {
        return $this->endpoint ?? (string) config('services.groq.chat_completions_url');
    }
    ...
    private function getApiKey(): string
    {
        ...
        if (! is_string($apiKey) || trim($apiKey) === '') {
            throw new GroqTranscriptCleanerException(ServiceUserMessage::missingApiKey('Groq'));
        }
    ...
    private function postChatCompletion(array $payload): Response
    {
        ...
        Log::info('Groq transcript cleaner request started.', $logContext);

        try {
            $response = $this->client()
                ->retry(
                    app(AppSettingsService::class)->groqMaxRetries(),
                    1000,
            ...
            Log::error('Groq transcript cleaner request timed out.', array_merge($logContext, [
        ...
        if ($response->failed()) {
            Log::error('Groq transcript cleaner request failed.', array_merge($logContext, [
                'elapsed_ms' => $this->elapsedMs($startedAt),
    ...
    private function assertResponseCompleted(Response $response): void
    {
        ...
        if (in_array($finishReason, ['length', 'max_tokens', 'max_output_tokens'], true)) {
            throw new GroqTranscriptCleanerException(ServiceUserMessage::invalidCleanerResponse('Groq'));
        }
    }
    ...
    private function chatPayload(string $systemInstruction, array $userPayload, array $options): array
    {
        $payload = [
            ...
            'max_completion_tokens' => (int) config('services.transcript_polishing.max_output_tokens', 4096),
    ...
    private function normalizeCleanedChunks(string $content, array $sourceChunks): array
    {
        ...
        foreach ($sourceChunks as $sourceChunk) {
            ...
            if (! isset($cleaned[$sourceId])) {
                throw new GroqTranscriptCleanerException(ServiceUserMessage::cleanerMissingChunks('Groq'));
            }
    ...
    private function userMessageForFailedResponse(int $status): string
    {
        return match (true) {
            ...
            default => ServiceUserMessage::cleanerFailed('Groq'),
        };
    }
}
