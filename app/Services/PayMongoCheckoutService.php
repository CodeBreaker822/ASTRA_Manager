...
class PayMongoCheckoutService
{
     ...
     */
    public function createCheckoutSession(User $user, string $planKey, array $plan, BillingTransaction $transaction): array
    {
        ...
        $amount = $this->amountFor($planKey);

        if ($amount <= 0) {
            throw new RuntimeException('PayMongo amount is not configured for this plan.');
        }
            ...
            ->post($this->apiUrl().'/v2/checkout_sessions', [
                'data' => [
                    'attributes' => [
                        ...
                        'description' => 'JERVA '.$plan['name'].' Plan',
                        'line_items' => [[
                            'name' => 'JERVA '.$plan['name'].' Plan',
                            'amount' => $amount,
                            'currency' => 'PHP',
                            'quantity' => 1,
                        ]],
    ...
    public function amountFor(string $planKey): int
    {
        return (int) config("services.billing.{$planKey}_amount", 0);
    }

    public function isConfiguredFor(string $planKey): bool
    {
        return is_string(config('services.paymongo.secret_key'))
            && config('services.paymongo.secret_key') !== ''
            && $this->amountFor($planKey) > 0;
    }
