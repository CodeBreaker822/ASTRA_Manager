...
use App\Exceptions\DeepSeekTranscriptCleanerException;
...
class DeepSeekTranscriptCleanerService
{
    public const MODEL_V4_FLASH = 'deepseek-v4-flash';
    ...
    public function clean(string $text, array $timestamps = [], array $options = []): array
    {
        ...
        if (! is_string($content) || trim($content) === '') {
            throw new DeepSeekTranscriptCleanerException(ServiceUserMessage::emptyCleanerResponse('DeepSeek'));
        }
    ...
    private function client(): PendingRequest
    {
        return Http::withToken($this->getApiKey())
            ->acceptJson()
            ->asJson()
            ->timeout($this->timeout ?? app(AppSettingsService::class)->deepSeekTimeout());
    }

    private function getEndpoint(): string
    {
        return $this->endpoint ?? (string) config('services.deepseek.chat_completions_url');
    }

    private function getModel(): string
    {
        $model = $this->model ?? app(AppSettingsService::class)->deepSeekModel();
        $allowedModels = config('services.deepseek.models', [self::MODEL_V4_FLASH]);

        if (! is_string($model) || ! in_array($model, $allowedModels, true)) {
            throw new DeepSeekTranscriptCleanerException(ServiceUserMessage::unsupportedProviderModel('DeepSeek'));
        }
    ...
    {
        $apiKey = $this->apiKey
            ?? app(AppSettingsService::class)->deepSeekApiKey()
            ?? config('services.deepseek.key');

        if (! is_string($apiKey) || trim($apiKey) === '') {
            throw new DeepSeekTranscriptCleanerException(ServiceUserMessage::missingApiKey('DeepSeek'));
        }
    ...
    private function postChatCompletion(array $payload): Response
    {
        ...
        Log::info('DeepSeek transcript cleaner request started.', $logContext);

        try {
            $response = $this->client()
                ->retry(
                    app(AppSettingsService::class)->deepSeekMaxRetries(),
                    1000,
            ...
            Log::error('DeepSeek transcript cleaner request timed out.', array_merge($logContext, [
                'elapsed_ms' => $this->elapsedMs($startedAt),
                'error' => $exception->getMessage(),
            ]));

            throw new DeepSeekTranscriptCleanerException(
                ServiceUserMessage::cannotReachProvider('DeepSeek'),
                0,
                $exception,
            );
        }
        ...
        if ($response->failed()) {
            Log::error('DeepSeek transcript cleaner request failed.', array_merge($logContext, [
                'elapsed_ms' => $this->elapsedMs($startedAt),
    ...
    private function assertResponseCompleted(Response $response): void
    {
        ...
        if (in_array($finishReason, ['length', 'max_tokens', 'max_output_tokens'], true)) {
            throw new DeepSeekTranscriptCleanerException(ServiceUserMessage::invalidCleanerResponse('DeepSeek'));
        }
    }
    ...
    private function normalizeCleanedChunks(string $content, array $sourceChunks): array
    {
        ...
        foreach ($sourceChunks as $sourceChunk) {
            ...
            if (! isset($cleaned[$sourceId])) {
                throw new DeepSeekTranscriptCleanerException(ServiceUserMessage::cleanerMissingChunks('DeepSeek'));
            }
    ...
    {
        return match (true) {
            in_array($status, [401, 403], true) => ServiceUserMessage::providerRejectedKey('DeepSeek'),
            $status === 429 => ServiceUserMessage::providerBusy('DeepSeek'),
            $status >= 500 => ServiceUserMessage::providerUnavailable('DeepSeek'),
            default => ServiceUserMessage::cleanerFailed('DeepSeek'),
        };
    }
}
