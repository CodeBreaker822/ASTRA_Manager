{{-- $overview holds the formatted figures; $analytics stays raw. See App\Support\DashboardOverview. --}}
            ...
            <section class="space-y-5 text-slate-900 dark:text-slate-100">
                <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="inline-flex rounded-md border border-slate-200 bg-white p-0.5 dark:border-slate-700 dark:bg-slate-900">
                            @foreach ($dayOptions as $days)
                                <a href="/dashboard?days={{ $days }}"
                                   @class([
                                       'rounded px-2.5 py-1.5 text-xs font-medium',
                                       'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900' => $overview['days'] === $days,
                                       'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white' => $overview['days'] !== $days,
                                   ])>{{ $days }}d</a>
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-2 overflow-hidden rounded-lg border border-slate-200 bg-white lg:grid-cols-4 dark:border-slate-700 dark:bg-slate-900">
                    <div class="border-r border-b border-slate-200 p-4 lg:border-b-0 dark:border-slate-700">
                        <p class="text-xs font-medium text-slate-500 dark:text-slate-400">Realized credits · lifetime</p>
                    ...
                    <div class="border-b border-slate-200 p-4 lg:border-r lg:border-b-0 dark:border-slate-700">
                        <p class="text-xs font-medium text-slate-500 dark:text-slate-400">Refundable balances · current</p>
                    ...
                    <div class="border-r border-slate-200 p-4 dark:border-slate-700">
                        <p class="text-xs font-medium text-slate-500 dark:text-slate-400">Paid top-ups · lifetime</p>
                    ...
                    <div class="p-4">
                        <p class="text-xs font-medium text-slate-500 dark:text-slate-400">Registered users · total</p>
                ...
                @if ($overview['reconciliation']['needed'])
                    <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
                ...
                <div class="grid gap-5 xl:grid-cols-3">
                    <section class="rounded-lg border border-slate-200 bg-white p-4 xl:col-span-2 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <div class="mt-5 overflow-x-auto pb-1">
                            <div class="flex h-40 min-w-[560px] items-end gap-2 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <div class="mt-4 grid gap-3 text-xs sm:grid-cols-3">
                            <div class="rounded-md bg-slate-50 p-3 dark:bg-slate-800/70">
                                <span class="text-slate-500 dark:text-slate-400">Exact tracked charges</span>
                                <p class="mt-1 font-semibold">{{ $overview['tracked_charges'] }}</p>
                            </div>
                            <div class="rounded-md bg-slate-50 p-3 dark:bg-slate-800/70">
                                <span class="text-slate-500 dark:text-slate-400">Pending checkouts</span>
                                <p class="mt-1 font-semibold">{{ $overview['pending_topups'] }}</p>
                            </div>
                            <div class="rounded-md bg-slate-50 p-3 dark:bg-slate-800/70">
                                <span class="text-slate-500 dark:text-slate-400">Realized basis</span>
                ...
                <section class="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
                    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Latest confirmed wallet funding</p>
                        </div>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                @forelse ($overview['recent_sales'] as $sale)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2.5 text-right font-semibold whitespace-nowrap text-emerald-600 dark:text-emerald-400">
                                            {{ $sale['amount'] }}
                                        </td>
                                    </tr>
                                @empty
                    ...
                    @foreach ($shortcuts as $shortcut)
                        <a href="{{ $shortcut['href'] }}"
                           class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 transition hover:border-blue-300 hover:bg-blue-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-700 dark:hover:bg-blue-950/30">
                            <x-icon :name="$shortcut['icon']" class="size-4 text-blue-600 dark:text-blue-400" />
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-semibold text-slate-950 dark:text-slate-100">{{ $shortcut['title'] }}</p>
                                <p class="truncate text-xs text-slate-500 dark:text-slate-400">{{ $shortcut['body'] }}</p>
                            </div>
