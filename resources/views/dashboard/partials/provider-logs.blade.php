{{-- Rows arrive display-ready from APIController::transcriptionProviderLogs(). --}}
@if ($logs->isEmpty())
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <div class="max-h-[58vh] overflow-y-auto">
            <table class="min-w-full divide-y divide-gray-200 text-left text-sm dark:divide-gray-700">
                <thead class="sticky top-0 bg-gray-50 text-xs tracking-wide text-gray-500 uppercase dark:bg-gray-900 dark:text-gray-400">
                    <tr>
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                    @foreach ($logs as $index => $log)
                        <tr class="align-top">
                            ...
                            <td class="px-3 py-3 whitespace-nowrap text-gray-700 dark:text-gray-200">{{ $log['source'] }}</td>
                            <td class="px-3 py-3">
                                <p class="font-medium text-gray-900 dark:text-gray-100">{{ $log['provider'] ?: 'Unknown provider' }}</p>
                                <p class="mt-0.5 max-w-64 text-xs break-all text-gray-500 dark:text-gray-400">{{ $log['model'] ?: 'Unknown model' }}</p>
                            </td>
                            <td class="px-3 py-3 whitespace-nowrap">
                                <span @class([
                                    'inline-flex rounded-full px-2 py-0.5 text-xs font-medium',
                                    'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300' => $log['succeeded'],
                                    'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' => ! $log['succeeded'],
                                ])>{{ $log['status_label'] }}</span>
                            </td>
                            <td class="px-3 py-3 whitespace-nowrap text-gray-600 dark:text-gray-300">{{ $log['fallback_position'] ?: '?' }}</td>
                            <td class="px-3 py-3 whitespace-nowrap text-gray-600 dark:text-gray-300">{{ $log['http_status'] ?: '—' }}</td>
                            <td class="max-w-72 px-3 py-3 text-gray-600 dark:text-gray-300">
                                @if ($log['error'])
                                    <span class="text-red-700 dark:text-red-300">{{ $log['error'] }}</span>
                                @else
                                    <span>—</span>
                                @endif
                            </td>
                ...
                @foreach ($pageNumbers as $page)
                    <button type="button" data-log-page="{{ $page }}"
                            @class([
                                ...
                                'border-gray-300 text-gray-700 dark:border-gray-600 dark:text-gray-200' => $page !== $pagination['current_page'],
                            ])
